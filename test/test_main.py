...
def test_backlog(client):
    client.get('/api/v1/greet/?name=test&format=json')
    client.get('/api/v1/greet/?name=test&format=plain')
...
def test_response_to_head(client):
    client.head('/api/v1/greet/?name=test&format=json')
    [exch1] = django_httpolice.backlog
    assert not notice_ids(exch1.responses[0])


def test_read_request_data(client):
    client.post('/api/v1/stream-greeting/',
                data=b'<hello/>', content_type='text/xml')


def test_streaming_response(client):
    client.get('/api/v1/stream-greeting/?n=4')
    [exch1] = django_httpolice.backlog
    # No notice 1039 because we don't read the response body.
    assert not notice_ids(exch1.responses[0])


def test_silence_globally(client):
    with override_settings(HTTPOLICE_SILENCE=[]):
        client.get('/api/v1/greet/?name=test')
        [exch1] = django_httpolice.backlog
